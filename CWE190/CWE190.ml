...
let rec backtrack_sub sub exp pos =
  let _ = return := Option.some exp in
  ...
  let _ = Seq.iter blk_t ~f:(fun blk ->
    ...
    Seq.iter def_t ~f:(fun def ->
      ...
      match exp with
      | BinOp (binop,exp1,exp2) -> 
        if parse rhs = Option.some exp && Tid.(>) (Term.tid pos) (Term.tid def) then
          let _ = new_track := parse_new rhs in
          backtrack_pos := Option.some def
        else if Tid.(=) (Term.tid pos) (Term.tid def) then
          if Option.is_some !new_track then
            return := backtrack_sub sub (Option.value_exn !new_track) (Option.value_exn !backtrack_pos)
      ...
      | Int word -> return := None
      | _ -> return := Option.some exp
    ) (*def*)
  ) in (*blk*)
  !return
...
let analyze_sub sub = 
  ...
  Seq.iter blk_t ~f:(fun blk ->
    ...
    Seq.iter def_t ~f:(fun def ->
      let rhs = Def.rhs def in
      let lhs = Def.lhs def in
      let binop = parse_binop rhs in
      if Option.is_some binop && Var.name lhs <> "RSP" then
        match Option.value_exn binop with
        | BinOp (binop,exp1,exp2) -> 
          if Option.is_some (parse exp1) || Option.is_some (parse exp2) then
            if backtrack_sub sub (Option.value_exn (parse exp1)) def <> None ||
               backtrack_sub sub (Option.value_exn (parse exp1)) def <> None then
              printf "Integer Overflow: %a" Def.ppo def
        | _ -> ()
    ) (*def*)
  ) (*blk*)
...
(* 如果是 jmp table 那么 更新 real_sub_name 返回 true 否则 返回false *)
(*  if
  1.#434724 := RBP
  2.RSP := RSP - 8
