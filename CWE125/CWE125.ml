...
let analyze_sub sub = 
  ...
  Seq.iter blk_t ~f:(fun blk ->
    ...
    Seq.iter def_t ~f:(fun def ->
      let rhs = Def.rhs def in
      let load = parse_load rhs in
      if Option.is_some load && Exp.to_string(Option.value_exn load) <> "RSP" then
        let _ = printf "\nREAD SIGNATURE: [%a]\n" Exp.ppo rhs in
        if backtrack_sub sub (Option.value_exn load) def <> None then
          let _ = return := None in
          printf "Out-of-bounds Read: %a" Def.ppo def
    ) (*def*)
  ) (*blk*)
